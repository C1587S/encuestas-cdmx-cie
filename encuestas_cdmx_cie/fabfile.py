...
${prefix}_DATABASE_NAME=encuestas_cdmx_cie
${prefix}_DATABASE_USER=encuestas_cdmx_cie
...
def defaults():
    ...
    env.code_dir = '/srv/encuestas_cdmx_cie'
        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.encuestas_cdmx_cie.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.encuestas_cdmx_cie.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "common.encuestas_cdmx_cie.node.include",
                "filename": "common.encuestas_cdmx_cie.node.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "common.encuestas_cdmx_cie.django.include",
                "filename": "common.encuestas_cdmx_cie.django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.encuestas_cdmx_cie.proxy_django.include",
                "filename": "app.encuestas_cdmx_cie.proxy_django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.encuestas_cdmx_cie.proxy_node.include",
                "filename": "app.encuestas_cdmx_cie.proxy_node.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.encuestas_cdmx_cie.proxy.include",
                "filename": "app.encuestas_cdmx_cie.proxy.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.encuestas_cdmx_cie.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "default_site": True,
                "pattern": "%(target)s.conf",
                "filename": "encuestas_cdmx_cie",
...
def test():
    ...
    defaults()
    env.node_site = 'encuestas-cdmx-cie.test.encuestascdmx.com'
    env.django_site = 'office.encuestas-cdmx-cie.test.encuestascdmx.com'
    env.hosts = ['test.encuestascdmx.com']
...
def live():
    ...
    env.node_site = 'encuestascdmx.com'
    env.django_site = 'office.encuestascdmx.com'
    env.hosts = ['production.encuestascdmx.com']
...
def setup_server(id=None):
    ...
    django_settings_file = env.code_dir + '/encuestas_cdmx_cie/django.env'
    ...
    # Create database
    sudo('echo "CREATE DATABASE encuestas_cdmx_cie; '
         '      CREATE USER encuestas_cdmx_cie WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE encuestas_cdmx_cie to encuestas_cdmx_cie;" '
         '| docker exec -i postgres-{postgres_version} psql -U postgres'.format(db_password=db_password,
                                                                                postgres_version=env.postgres_version))
    # Create log dir
    sudo('mkdir -p /var/log/encuestas_cdmx_cie/')
    ...
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/encuestas_cdmx_cie')
...
def add_secret_keys(component=None):
    ...
    if component == 'node':
        ...
        remote_path = env.code_dir + '/encuestas_cdmx_cie/django.env'
...
def collectstatic():
    docker_compose_run('node', 'yarn export-assets', name='encuestas_cdmx_cie_yarn_export')
...
def docker_compose_run(service, cmd='', name='encuestas_cdmx_cie_tmp'):
...
def ensure_docker_networks():
    ...
    ensure_docker_network_exists('encuestas_cdmx_cie_default', [], internal=False)
    ensure_docker_network_exists('encuestas_cdmx_cie_nginx', ['nginx'])
    ensure_docker_network_exists('encuestas_cdmx_cie_postgres', ['postgres-{postgres_version}'.format(
        postgres_version=env.postgres_version)])
...
def ensure_local_py_exists():
    ...
    django_local_settings = env.code_dir + '/encuestas_cdmx_cie/settings/local.py'
